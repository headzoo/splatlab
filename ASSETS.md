...
| `neutral_rupert_01` | Rupert the Coop Keeper: young cream rooster with rusty-orange neck feathers, an unbrushed red crest, alert deadpan expression, rolled muted-teal sleeves, rugged brown trousers, scuffed boots with close-held loose lace ends, battered shoulder tool bag with cable/fuse details and a clipped lamp with one green LED, exactly three short charging-cable ends, mismatched belt keys, one taped wing feather, and a curled blank name-badge shape. Keep all gear inside the silhouette and present the literal name, shared flask, chalk marks, hillside story, and tagline in UI/world content rather than baked sprite text or scenery. Generate with the built-in image tool using `neutral_cooper_01-source.png` and `neutral_jamie_01-source.png` as Splat Lab style, camera, density, and `5 x 4` gait references; then process `sprite-specs/neutral_rupert_01.json` and promote only through Sprite Viewer approval. |
| `neutral_jamie_01` | Jamie the Framer Chicken: stocky white chicken with a calm squint and smirk, worn crooked tan cap, ochre-brown work vest over red-and-navy flannel, tool belt with speed square, chalk line, and holstered nail gun, sawdust-marked work boots, a chart-lit vest phone, and a tiny close-held laptop. Keep all tools inside the silhouette, omit readable text and scenery from the runtime sheet, and present the name and tagline in UI copy. Generate with the built-in image tool using `neutral_cooper_01-source.png` only as the authoritative Splat Lab style, camera, spacing, and `5 x 4` gait-layout reference; then process `sprite-specs/neutral_jamie_01.json` and promote only through Sprite Viewer approval. |
| `neutral_vix_01` | Vix: lean orange-russet fox with a cream muzzle and chest, sharp shoulders, raised eyebrow and sly smirk, nicked hoop-pierced ear, smudged eyeliner, cropped scuffed black leather jacket over a modest torn-edge black tank, combat boots, chain belt with lockpicks, harmless switchblade charm, blank hotel keycard, cracked cyan-lit jacket phone, crooked blank safety-pinned badge, and low curling tail. Keep the look mischievous and age-appropriate, keep all accessories inside the silhouette, and omit readable text, weapons in hand, and scenery from the runtime sheet. Present the fire-escape/neon identity and tagline in UI. Generate with the built-in image tool using the Cooper and Jamie source sheets as Splat Lab style and `5 x 4` gait-layout references; then process `sprite-specs/neutral_vix_01.json` and promote only through Sprite Viewer approval. |
| `neutral_leenie_01` | Leenie: poised cream-white hen with a neatly swept feather tuft, tidy red comb, long lashes, subtly raised brow, warm smile, blush cardigan over a crisp white collared blouse, tailored navy-and-pink plaid skirt, cream knee socks, glossy oxblood loafers, close-held pastel shoulder purse, compact mirror, and gold heart locket. Her polished, fashion-conscious silhouette should read as slightly snobby while her eyes and smile retain the big-hearted good-girl warmth. Keep accessories inside the silhouette and omit readable text, scenery, logos, and name labels. Generate with the built-in image tool as a transparent `5 x 4` Splat Lab locomotion master; process `sprite-specs/neutral_leenie_01.json` and promote only through Sprite Viewer approval. |
| `neutral_lango_01` | Lango: kid-friendly young rooster with warm cream face feathers, charcoal body feathers, a long messy deep-indigo crest swept over part of one eye, orange beak and three-toed feet, small wattle and tail, compact charcoal sleeveless hoodie, dark utility belt, scuffed black foot wraps, old smartphone, and a battered cross-body laptop bag with a short charging-cable loop and tiny improvised patch/tool details. Keep him focused and resourceful rather than gloomy, keep all gear inside the chicken silhouette, and omit human anatomy, readable text, scenery, logos, and name labels. Generate with the built-in image tool using the Cooper and Rupert source sheets as chicken anatomy/layout references and Vix only as an emo-streetwear reference; process `sprite-specs/neutral_lango_01.json` and promote only through Sprite Viewer approval. |
...
| `neutral_rupert_01_defeated` | Four left and four right harmless defeat frames for Rupert: startled step, knees buckling, controlled low collapse, and resting dazed pose, with the tool bag and repair gear retained and grounded at `(32, 56)`, 10 FPS play once. Generate with the built-in image tool from Rupert's locomotion master and the existing Jamie defeated layout. Keep each figure wholly inside its own source cell; this source's deliberately irregular transparent row gutters use the reviewed per-frame `sourceLayout.regions` in `sprite-specs/neutral_rupert_01.json` so frame 1 cannot absorb frame 5's crest. Process and approve atomically as the `defeated` event owned by `neutral_rupert_01`. Rupert's player-hero trait remains unkillable, so this role-neutral event art is not selected during normal hero play. |
| `neutral_jamie_01_defeated` | Four left and four right harmless defeat frames for Jamie: startled stumble, knees buckling, controlled collapse, and resting dazed pose, with all construction gear retained and grounded at `(32, 56)`, 10 FPS play once. Generate with the built-in image tool from Jamie's locomotion master and the existing Cooper defeated layout, retain the recipe's reviewed manual source regions so adjacent rows cannot bleed together, then process and approve as the `defeated` event owned by `neutral_jamie_01`. |
| `neutral_vix_01_defeated` | Four left and four right harmless defeat frames for Vix: surprised stumble, tail-braced buckle, controlled low fall, and resting dazed pose, with the tail and streetwear accessories retained and grounded at `(32, 56)`, 10 FPS play once. Generate with the built-in image tool from Vix's locomotion master and the existing human defeated layout, retain the recipe's reviewed manual source regions so frame 1 cannot inherit frame 5 pixels and frame 5 keeps its complete head, then process and approve as the `defeated` event owned by `neutral_vix_01`. |
| `neutral_leenie_01_defeated` | Four left and four right harmless defeat frames for Leenie: surprised stumble, composed kneel, controlled low fall, and resting dazed pose, with her complete head, comb, skirt, purse, locket, and footwear retained and grounded at `(32, 56)`, 10 FPS play once. Generate with the built-in image tool from Leenie's locomotion master, retain the recipe's reviewed manual `y=452` source-row boundary so the first right-facing frame keeps its full head, then process and approve atomically as the `defeated` event owned by `neutral_leenie_01`. |
| `neutral_lango_01_defeated` | Four left and four right harmless defeat frames for Lango: surprised backward step, knees buckling, wing-braced low fall, and resting dazed pose. Preserve his full swept indigo crest, cream-and-charcoal feathers, phone, hoodie, patched laptop bag, and charging-cable loop; keep every pose inside its own cell and grounded at `(32, 56)`, 10 FPS play once. Generate with the built-in image tool from Lango's locomotion master and the existing chicken defeat sheets, remove any floating dizzy symbols, then process and approve atomically as the `defeated` event owned by `neutral_lango_01`. |
...
| `space_platformer_hud_lives_01` | Single `64 x 64` lives portrait, space/Cooper-friendly, centered, with no text baked in. This is HUD art only. |
| `shared_platformer_easter_egg_01` | Single static `64 x 64` shared extra-life pickup: one upright, complete decorated Easter egg in bright chunky pixel art, with a dark navy outline and saturated turquoise, yellow, berry-pink, and orange bands, zigzags, and dots. Recreate from `sprites/shared_platformer_easter_egg_01-source.png` using the matching recipe, keep genuine transparency and one centered subject with no text, UI, scenery, shadow, glow field, basket, grass, flowers, bunny, chick, or character, then stage and review it through the Sprite Viewer before approval. |
...
| `brand/cooper/cooper.png` | `1024 x 1024` opaque Team-page Cooper character card generated with the built-in image generator using `brand/cooper/rupert.png` and `brand/cooper/jamie.png` as composition/style references and `brand/cooper/cooper-1.png` as the authoritative Cooper identity. Recreate Cooper in a chaotic hillside invention lab with a bubbling green flask, small toy cannon, slime, blueprints, chicken-comb castle cottages, a wooden `Cooper` name sign, and a smaller plank reading `Should we test it?`. Preserve cream-white feathers, huge blue-black goggles, white lab coat, `Cooper` badge, pocket pens, exactly two peach bandages crossed on the beak, one small red X scratch on each cheek, a clean remaining face, and only faint extra slapstick scuffs. Exclude gore, extra facial wounds, photorealism, UI, and watermarks. Convert the generated JPEG to PNG without resampling. |
| `brand/cooper/rupert.png` | `1208 x 1302` opaque Team-page Rupert character card. Recreate with the built-in image generator as a night hillside-village scene: cream rooster with rusty-orange neck feathers, teal work shirt, brown trousers, boots, taped wing, tool bag, keys, flask, wooden `Rupert` sign, and quote plank `I'll be there in ten.` Preserve the dense comic-card layout, kid-friendly lighting, and no gore. |
| `brand/cooper/jamie.png` | `1208 x 1302` opaque Team-page Jamie character card. Recreate as a daytime construction-frame scene: stocky white chicken with tan cap, red-navy flannel, ochre vest, tool belt, blueprints, laptop, wooden `Jamie` sign, and quote `Frames houses by day. Frames trades by night.` |
| `brand/cooper/vix.png` | `1208 x 1302` opaque Team-page Vix character card. Recreate as a night fire-escape alley: orange-russet fox in a scuffed black jacket, chain belt, combat boots, wooden `Vix` sign, and quote `Wasn't here.` Keep the look mischievous and age-appropriate. |
| `brand/cooper/lango.png` | `1208 x 1302` opaque Team-page Lango character card. Recreate as a late-night cluttered workshop: charcoal-and-cream rooster with a messy indigo crest, beat-up laptop, crates, mugs, and a `Lango` name sign. Preserve `Full send` / no-degree workshop copy as painted props. |
| `brand/cooper/leenie.png` | `1208 x 1302` opaque Team-page Leenie character card. Recreate as a sunny schoolyard locker scene: cream hen in a pink cardigan, plaid skirt, heart mirror, pink lockers, and a `Leenie` name treatment. Keep her polished and big-hearted. |
| `brand/cooper-studio-apartment-lab.png` | `1000 x 800` environment illustration of Cooper's open-plan studio apartment and personal laboratory, generated with the built-in image editor using the first apartment-lab draft as the composition target and `brand/cooper-profile-banner.png` as the authoritative style reference. Recreate as one cohesive room with a red beanbag sleeping nook, blue sleeping cap, books, pizza and tiny kitchenette on the left; a Green Hills window view in the center; and a wooden lab bench with green-slime glassware, blueprints, game-controller diagrams, laptop, lamp, oversized tools, small red-and-cream test cannon, chalkboard diagrams, jars, parts, and pipes on the right. Match the banner's smooth rounded geometry, crisp outlines, simplified silhouettes, saturated cyan/lime/gold/red palette, glossy highlights, cool soft shadows, bright daylight, and polished children's platform-game key-art finish. Keep both home and lab zones readable; exclude characters, comic panels, speech bubbles, readable words, logos, UI, borders, and watermarks. |
...
| `brand/cooper/cooper/*.png` | Twelve `1000 x 1000` opaque campaign illustrations remade with the built-in image editor. Use each previous image as the authoritative scene, camera, pose, prop, headline-copy, and hierarchy target; use `brand/cooper/cooper-1.png` and `brand/cooper/cooper-5.png` as the authoritative updated Cooper design, and `brand/cooper-profile-banner.png` as the shared polished 2D storybook-comic style reference. Cooper must retain cream-white feathers, glossy red comb and wattles, huge blue-black goggles, orange beak and feet, white lab coat, blue-bordered `Cooper` badge, red/blue/yellow pens, exactly two small peach bandages crossed on top of the beak, exactly one small thin red cross-scratch on each cheek, an otherwise clean face, and only faint pale coat scuffs. Preserve each original story: `GLPgsEr9vgzov1Lyplx8.png` increase-power machine; `IA4lGF9QMYkGECq6ibNk.png` three game-idea thought panels; `K58RG1Ify82kT3UE6Yik.png` creator-loop poster; `KwwaonKL1aXZXJmNRv8L.png` child/Cooper cardboard-robot high-five; `MkFx8UE7pQcLQetZPeiM.png` four-game collage; `PKBBM0PsWnrcMb7mAJz2.png` cannon flight test; `SvThc1agrePkFA4MY7HV.png` colorful beaker incident; `YTBejha1NCzP3Z5F2er4.png` guarded red button; `lfmsVtUZJesYjchD9mz5.png` nervous cannon-horn experiment; `lik5oNaD5zvoJECRSruH.png` wheeled cardboard robot reveal; `uzEumn2ZWkrhnM15ghKT.png` kid-ideas campaign poster; and `vccw4GE1CFlg1wTdXmYv.png` chicken-versus-guinea-pig comparison. Remove every generated/fake Splat Lab mark during generation, leave a clean integrated backing surface at each former logo location, then composite a proportionally scaled exact copy of `brand/logo.png` into that location; `lfmsVtUZJesYjchD9mz5.png` intentionally remains logo-free because its source has no prominent brand placement. Supporting chickens must remain distinct and must not inherit Cooper's bandages, cheek scratches, badge, or exact goggles. Preserve named headline text, simplify illegible incidental microcopy into non-text marks rather than gibberish, and exclude extra wounds, brown facial grime, fake logos, misspelled brand marks, photorealism, 3D rendering, and watermarks. |
| `brand/splat-lab-kids-make-their-own-game-tweet.png` | `1600 x 900` opaque social illustration generated with the built-in image editor from the kid-ideas campaign artwork, the two updated Cooper references, and `brand/cooper-profile-banner.png`. The child is the unmistakable author: one hand operates the mouse to place a grass block while the other points to a hand-drawn level sketch whose platforms, coin, friendly creature, obstacle, and small castle match the on-screen Green Hills level. Cooper remains secondary, stands away from every control, and only cheers while holding a tray of selectable game pieces. Preserve the saturated cyan/lime/gold/red 2D storybook-comic finish, clean warm outlines, updated Cooper bandages and cheek scratches, and full-bleed 16:9 composition. Generate without words or branding, then composite an untouched proportionally scaled copy of `brand/logo.png` into the clear upper-right sky. Exclude tweet copy, readable UI text, fake logos, AI avatars, robots, prompt boxes, chat bubbles, magic wands, autopilot imagery, watermarks, borders, and Cooper touching the child's controls. |
| `brand/splat-lab-what-would-be-fun-tweet.png` | `1600 x 900` opaque companion social illustration generated with the built-in image editor from `brand/splat-lab-kids-make-their-own-game-tweet.png`, the two updated Cooper references, and `brand/cooper-profile-banner.png`. Show the same red-hoodie child holding a blue controller and actively playing a game that began as their exuberant crayon sketch: a rocket-boot hero, an arc of golden rings, a smiling green slime trampoline, and a friendly red dragon must match clearly between the foreground sketchbook and the large Green Hills game screen. Use colorful sketch sparks to connect the drawing to the playable result. Keep Cooper secondary, standing behind the child with a thumbs-up and touching no controls. Preserve the saturated 2D storybook-comic rendering, updated Cooper bandages and cheek scratches, full-bleed 16:9 framing, and clean upper-right sky. Generate without words or branding, then composite an untouched proportionally scaled copy of `brand/logo.png` into the reserved sky. Exclude code, syntax, terminals, readable UI text, tweet copy, fake logos, AI avatars, robots, prompt boxes, chat bubbles, magic wands, watermarks, borders, and Cooper touching the child's controls. |
| `brand/splat-lab-what-if-tweet.png` | `1600 x 900` opaque companion social illustration generated with the built-in image editor from `brand/splat-lab-what-would-be-fun-tweet.png`, the two updated Cooper references, and `brand/cooper-green-hills-world.png`. Show the red-hoodie child originating three exuberant ideas while Cooper listens and holds matching pictorial cards. Turn the ideas into one continuous Green Hills panorama: a pale chicken-comb castle rolling on giant wooden wagon wheels; a smiling red-orange dragon in a blue hero cape and golden shield rescuing two fluffy chicks across a broken bridge; and a far-right vertical cliff zone where a glowing horizontal arrow, sideways waterfall, coins, leaves, crates, house, platforms, and game hero make sideways gravity unmistakable. Repeat the three ideas as simple crayon drawings in the foreground sketchbook. Preserve the saturated 2D storybook-comic rendering, updated Cooper bandages and cheek scratches, full-bleed 16:9 framing, and reserved upper-center-right sky. Generate without words or branding, then composite an untouched proportionally scaled copy of `brand/logo.png` into the reserved sky. Exclude ordinary castle staging, villainous dragons, frightened chicks, ambiguous gravity, code, syntax, readable UI text, tweet copy, fake logos, AI avatars, robots, prompt boxes, chat bubbles, watermarks, borders, and Cooper directing the child's ideas. |
| `brand/splat-lab-no-plan-family-project-tweet.png` | `1600 x 900` opaque companion social illustration generated with the built-in image editor from the cardboard-robot campaign artwork, `brand/splat-lab-what-if-tweet.png`, and the two updated Cooper references. Show the red-hoodie child and one parent building together in a sunny family room while Cooper joins them at floor level, happily tangled in masking tape. Center an intentionally unclassifiable handmade contraption assembled from cardboard boxes, paper tubes, tape, bottle caps, string, popsicle sticks, mismatched wheels, painted dragon wings, a castle turret, paper-plate propeller, kitchen colander, game controller, craft lights, and loose wires. The child paints while the parent tapes on a wheel; an open laptop uses only pictorial colored code blocks to drive harmless lights, bubbles, confetti, and the spinning propeller. Scatter paper, paint, crayons, child-safe scissors, tape, and loose sketches that separately imagine the object as a rocket, castle, dragon, and robot, with no formal blueprint. Preserve the saturated 2D storybook-comic rendering, updated Cooper bandages and cheek scratches, tactile craft materials, full-bleed 16:9 framing, and reserved upper-right window area. Generate without words or branding, then composite an untouched proportionally scaled copy of `brand/logo.png` into the reserved area. Exclude a polished single-purpose machine, tidy empty workspace, child working alone, adult directing the child, readable code, terminals, tweet copy, fake logos, AI avatars, prompt boxes, chat bubbles, dangerous tools, scary malfunction, watermarks, and borders. |
| `brand/splat-lab-build-together-constraints-tweet.png` | `1600 x 900` opaque companion social illustration generated with the built-in image editor from `brand/splat-lab-no-plan-family-project-tweet.png`, `brand/splat-lab-what-if-tweet.png`, and the two updated Cooper references. Show the same red-hoodie child and green-sweater parent as equal collaborators building a sprawling handmade marble-run/game course. Put a neat square course plan and dark-blue taped floor boundary in the foreground, then make the actual red, yellow, and cyan track escape the boundary, climb over a cardboard wall, form a huge loop, cross a gap on suspended painted cups, and end at a tiny cardboard castle. The child launches a glowing golden ball; the smiling parent braces the wild loop with a triangular cardboard support and repurposes a metal ruler as its ramp; Cooper waits at the finish with a small net. Preserve the saturated 2D storybook-comic rendering, tactile cardboard and tape, updated Cooper bandages and cheek scratches, full-bleed 16:9 framing, and clean upper-right sky. Generate without words or branding, then composite an untouched proportionally scaled copy of `brand/logo.png` into the reserved sky. Exclude adults scolding or directing, children working alone, unsafe climbing, failed machinery, ordinary straight tracks, readable instructions, code, tweet copy, fake logos, AI avatars, prompt boxes, chat bubbles, watermarks, borders, and extra Cooper wounds. |
| `brand/splat-lab-creativity-is-practice-tweet.png` | `1600 x 900` opaque companion social illustration generated with the built-in image editor from `brand/splat-lab-no-plan-family-project-tweet.png`, `brand/splat-lab-build-together-constraints-tweet.png`, and the two updated Cooper references. Show the same red-hoodie child and green-sweater parent practicing creativity together during a long afternoon. Arrange four earlier handmade projects on the upper-left shelves in a clear progression from a paper cup on bottle-cap wheels to a one-eyed box creature, a colorful robot, and a looping cardboard course; include a numeral-free late-afternoon wall clock. Center the current strange creature-machine with spring legs, googly eyes, xylophone teeth, mismatched painted wings, colander dome, game controller, horn, pinwheel, flower, craft lights, and plentiful cardboard, tape, paint, paper, crayons, clothespins, yarn, and safe scissors. The child turns a crank while the parent tapes on a wing. Cooper waits behind a clear tabletop shield with oversized earmuffs and a small extinguisher, but the machine produces only bubbles, petals, and one musical-note symbol. Preserve the saturated 2D storybook-comic rendering, tactile materials, updated Cooper bandages and cheek scratches, full-bleed 16:9 framing, and clean upper-right window. Generate without words or branding, then composite an untouched proportionally scaled copy of `brand/logo.png` into the reserved window. Exclude awards, grading, innate-talent symbolism, adults directing, children working alone, polished products, real explosions, fire, smoke, sparks, chemicals, readable text or code, fake logos, AI avatars, prompt boxes, chat bubbles, watermarks, borders, and extra Cooper wounds. |
| `brand/splat-lab-smartest-chicken-failure-tweet.png` | `1600 x 900` opaque social illustration generated with the built-in image editor from the beaker-incident campaign artwork, `brand/splat-lab-creativity-is-practice-tweet.png`, and the two updated Cooper references. Make the tone dry, sarcastic, and much less wholesome: late at night, center Cooper slouched on a rolling lab stool with a half-lidded deadpan expression, dented coffee mug, clipboard covered in red X marks with one tiny green check, crooked paper crown, and bottle-cap medal. Surround him with the accumulated evidence of failed experiments: eleven broken hanging bulbs and one working bulb, slime dripping from the ceiling fan, overflowing purple foam, bent wheeled cannon, spring-loaded toaster and toast stuck in the wall, upside-down robot, wheeled cardboard castle, loose papers, parts, spills, and two skeptical cleanup chickens. Preserve Cooper's updated crossed beak bandages, small red cross-scratch on each cheek, huge goggles, lab coat, badge, pens, and otherwise clean face; allow only light coat scuffing. Keep the failures chaotic but harmless, with no children or parents. Generate without words or branding, then composite an untouched proportionally scaled copy of `brand/logo.png` into the reserved upper-right dark-blue panel. Exclude wholesome family bonding, active explosions, flames, injuries, gore, sharp glass, fake logos, readable text, watermarks, borders, and extra Cooper wounds. |
...
| `brand/cooper-green-hills-world.png` | `2000 x 1500` Green Hills chicken-civilization world illustration generated with the built-in image editor from the original environment-only vista. Preserve the exact 4:3 valley composition, saturated children's game-art style, grass-topped warm-rock plateaus, turquoise waterfalls, winding river, foliage, evergreen forests, blue snowcapped mountains, clouds, and atmospheric depth. Populate multiple hilltops with small clusters of warm timber-and-pale-stone chicken-scale cottages featuring rounded coop-inspired red roofs, egg-shaped openings, perch-like porch rails, rooster or feather weathervanes, winding paths, footbridges, and soft white chimney smoke. Keep the pale-stone castle in the upper-right distance but replace its ordinary cones with rounded, floppy red steeple lobes shaped unmistakably like chicken combs; add subtle chicken crests and egg-shaped windows. Exclude giant chickens, people, other animals, readable text, logos, UI, coins, cannons, nets, comic panels, borders, fire, destruction, and watermarks; resize the selected exact-4:3 source to `2000 x 1500`. |
| `brand/cooper-green-hills-castle-closeup.png` | `2000 x 1500` close three-quarter view of the Green Hills castle, generated with the built-in image editor using `brand/cooper-green-hills-world.png` as the authoritative world and style reference. Fill the frame with a grand pale-stone chicken-civilization castle above turquoise waterfalls, with rounded red chicken-comb steeples, egg-shaped windows, a chicken crest over the gate, feather banners, wooden bridges, courtyard terraces, smoking cottage chimneys, bright lime grass, and blue mountain depth. Preserve the same polished rounded 3D-cartoon children's key-art finish and exclude characters, animals, readable text, logos, UI, coins, cannons, nets, comic panels, borders, and watermarks. |
| `brand/cooper-green-hills-hillside-village.png` | `2000 x 1500` close Green Hills hillside-village scene, generated with the built-in image editor using `brand/cooper-green-hills-world.png` as the authoritative world and style reference. Recreate a lively terraced settlement of timber-and-pale-stone chicken cottages with red coop roofs, egg openings, perch rails, rooster weathervanes, chimney smoke, winding paths, gardens, fences, small bridges, and an egg-shaped village fountain, with the chicken-comb castle visible in the distance. Preserve the same saturated palette, rounded geometry, glossy daylight, warm rock, lime grass, turquoise water, and polished children's game-art finish; exclude characters, animals, readable text, logos, UI, coins, cannons, nets, comic panels, borders, and watermarks. |
| `brand/cooper-green-hills-riverside.png` | `2000 x 1500` Green Hills riverside settlement, generated with the built-in image editor using `brand/cooper-green-hills-world.png` as the authoritative world and style reference. Center the composition on a winding turquoise river and tiered waterfalls, with a feather-paddle watermill, covered wooden bridge, docks, footbridges, red-roofed chicken cottages, egg openings, perch details, rooster weathervanes, chimney smoke, lush grass, warm rock cliffs, forests, and the comb-steepled castle in the distance. Preserve the same smooth rounded 3D-cartoon children's key-art style and exclude characters, animals, readable text, logos, UI, coins, cannons, nets, comic panels, borders, and watermarks. |
...
| `brand/mock-1-grass-block.png` | `1254 x 1254` transparent standalone grass block generated with the built-in image generator using `brand/mock-1.png` as the authoritative visual reference. Recreate exactly one centered, single-cell floating terrain block in three-quarter front view, with a vivid rounded lime-grass cap, short grass tufts, and a thick warm orange-brown faceted earth-and-rock body. Preserve generous transparent padding and exclude additional blocks, platforms, scenery, trees, flowers, coins, characters, ladders, crates, signs, text, logos, UI, checkerboards, and watermarks. |
| `brand/mock-1-coin.png` | `1254 x 1254` transparent standalone coin generated with the built-in image generator using `brand/mock-1.png` as the authoritative visual reference. Recreate exactly one centered, thick gold collectible coin in slight three-quarter view, with a bright yellow-gold face, beveled orange-gold rim, glossy upper-left highlights, warm shading, and the mockup's raised S-shaped currency mark with one vertical stroke. Preserve generous transparent padding and exclude additional coins, blocks, scenery, characters, text beyond the embossed symbol, logos, UI, loose sparkles, checkerboards, and watermarks. |
| `brand/mock-1-wooden-crate.png` | `1254 x 1254` transparent standalone wooden crate generated with the built-in image generator using `brand/mock-1.png` for the original crate design and `brand/mock-1-grass-block.png` plus `brand/mock-1-coin.png` for the finished object set's perspective, lighting, saturation, and polish. Recreate exactly one centered closed shipping crate in three-quarter front view with thick beveled honey-brown planks, a raised front X-brace, subtle warm wood grain, darker seams, and small charcoal metal fasteners. Preserve generous transparent padding and exclude grass, dirt, rocks, coins, contents, rope, labels, letters, symbols, characters, scenery, platforms, floor shadows, UI, checkerboards, and watermarks. |
| `brand/mock-2.png` | `1226 x 1283` product/art reference mockup for Splat Lab tone and layout. |
| `apps/website/public/brand/homepage/hero-background-clean.png` | `1585 x 992` fully opaque homepage scenery plate behind Cooper and every separate parallax layer, generated with the built-in image editor. Use the previous clean plate as the authoritative wide camera, expansive left/center sky, atmospheric depth, overlay-safe negative space, and canvas reference; use `brand/cooper-green-hills-hillside-village.png` as the authoritative chicken-civilization environment and style reference and `brand/cooper-green-hills-castle-closeup.png` as the authoritative castle reference. Preserve bright open cyan sky and soft clouds across the left and center, layered blue mountains, a deep green valley, turquoise rivers and waterfalls, and scenery concentrated along the lower and right portions. Populate the midground hills with small warm timber-and-pale-stone cottages featuring red coop roofs, egg openings, perch rails, weathervanes, paths, bridges, gardens, and chimney smoke. Place one complete pale-stone chicken castle on the upper-right ridge at medium-distant scale, with glossy rounded red tower crowns shaped unmistakably like chicken-comb lobes, egg-shaped windows, and a subtle chicken crest. Keep this as one flattened non-gameplay background and exclude wooden signs, ladders, crates, coins, slime, standalone grass blocks, floating platforms, builder previews, selection UI, characters, animals, cannons, nets, text, logos, borders, and watermarks so no moving parallax object is baked into the plate. Preserve the exact dimensions without resizing. |
...
| `apps/website/public/brand/homepage/icon-ideas.png` | `300 x 176` transparent homepage benefit illustration generated with the built-in image generator from the current homepage Cooper, Chicken Quest card, and Cooper apartment-lab references. Recreate Cooper holding a glowing blue game blueprint while one tiny lime-grass platform and gold coin materialize from golden sparks. Match the polished saturated Splat Lab game-art finish; exclude lightbulbs, generic clip-art, readable text, code, UI, scenery, checkerboards, and watermarks. Trim the selected source, fit it within `286 x 168`, and center it on the transparent canvas. |
| `apps/website/public/brand/homepage/icon-online.png` | `300 x 176` transparent homepage benefit illustration generated with the built-in image generator from the current homepage Cooper, Chicken Quest card, and Cooper apartment-lab references. Recreate one joyful group of Cooper, a blue space robot, a pale ghost, and a red-orange dragon celebrating around a glowing coin on one tiny lime-grass platform. Match the polished saturated Splat Lab game-art finish; exclude controllers, generic people icons, readable text, code, UI, scenery, checkerboards, and watermarks. Trim the selected source, fit it within `286 x 168`, and center it on the transparent canvas. |
| `apps/website/public/brand/homepage/icon-no-code.png` | `300 x 176` transparent homepage benefit illustration generated with the built-in image generator from the current homepage Cooper, Chicken Quest card, and Cooper apartment-lab references. Recreate Cooper pressing one oversized yellow lab button while two grass blocks, one four-point-sparkle coin, and one small blue goal flag pop into place with cyan and gold construction sparkles. Match the polished saturated Splat Lab game-art finish; exclude magic wands, pencils, currency marks, code symbols, keyboards, readable text, UI, scenery, checkerboards, and watermarks. Trim the selected source, fit it within `286 x 168`, and center it on the transparent canvas. |
| `apps/website/public/brand/homepage/icon-kids.png` | `300 x 176` transparent homepage benefit illustration generated with the built-in image generator from the current homepage Cooper, Chicken Quest card, and Cooper apartment-lab references. Recreate Cooper sharing a tiny lime-grass game world with two cheerful diverse children, with a subtle glossy heart shield behind them. Match the polished saturated Splat Lab game-art finish; exclude generic heart-only clip-art, controllers, code, readable text, UI, scenery, checkerboards, and watermarks. Trim the selected source, fit it within `286 x 168`, and center it on the transparent canvas. |
| `apps/website/public/brand/homepage/game-meadow-coin-quest.webp` | `720 x 403` live-game thumbnail for Meadow Coin Quest. Recreate by removing the `data:image/webp;base64,` prefix from `plan/thumb-1.txt` and Base64-decoding the remaining bytes without resizing or recompressing. Links to `/play/cmu0hdkqc000104jnc9od9t8c`. |
| `apps/website/public/brand/homepage/game-coopers-ghost-quest.webp` | `720 x 403` live-game thumbnail for Cooper's Ghost Quest. Recreate by removing the `data:image/webp;base64,` prefix from `plan/thumb-2.txt` and Base64-decoding the remaining bytes without resizing or recompressing. Links to `/play/cmu06onhe000004l5djp754cv`. |
| `apps/website/public/brand/homepage/game-coopers-star-mission.webp` | `720 x 403` live-game thumbnail for Cooper's Star Mission. Recreate by removing the `data:image/webp;base64,` prefix from `plan/thumb-3.txt` and Base64-decoding the remaining bytes without resizing or recompressing. Links to `/play/cmu07vdyc000004l404jcm0wj`. |
| `apps/website/public/brand/homepage/game-dragon-cooper-jump.webp` | `720 x 403` live-game thumbnail for Dragon Cooper Jump. Recreate by removing the `data:image/webp;base64,` prefix from `plan/thumb-4.txt` and Base64-decoding the remaining bytes without resizing or recompressing. Links to `/play/cmu0h41ng000004jn4er6ro6a`. |
| `apps/website/public/brand/homepage/cooper-hero-fixed.png` | `1032 x 1190` transparent homepage Cooper generated with the built-in image editor using the previous `cooper-hero-fixed.png` as the authoritative airborne pose and composition target, and `brand/cooper/cooper-1.png`, `brand/cooper/cooper-3.png`, `brand/cooper/cooper-4.png`, and `brand/cooper/cooper-5.png` as the updated character-design references. Preserve the energetic tilted head, wide-spread wings, dangling legs, open-beak expression, centered scale, transparent padding, blue-black goggles, white coat, `Cooper` badge, and red/blue/yellow pens. Use exactly two peach adhesive bandages crossed on top of the beak and exactly one small thin red cross-scratch on each cheek; keep the remaining face clean, allow only a few faint pale coat scuffs, and exclude every other bandage, scratch, wound, brown splatter, grime, prop, action ray, background, floor shadow, halo, border, logo, UI element, and watermark. Preserve genuine alpha, then normalize the selected exact-proportion result to `1032 x 1190`. |
| `apps/website/public/brand/site-header-logo.png` | `2067 x 634` transparent horizontal site-header lockup combining an updated Cooper mascot with the canonical `apps/game/logo-text.png` wordmark. Generate only the compact navbar Cooper with the built-in image editor, using the previous lockup as the authoritative head angle, raised-wing pose, feather silhouette, crop, and rendering reference and the updated homepage, Features, and Parents Cooper cutouts as facial-design references. Cooper must have exactly two small peach adhesive bandages crossed on top of his beak and exactly one small thin red cross-scratch on each cheek, with otherwise clean cream-white feathers; preserve the large blue-black goggles, glossy red comb and wattles, cheerful open beak, warm-dark outline, and absence of a lab coat or props. Require genuine alpha and exclude all text from the mascot generation. Trim the selected mascot, resize proportionally to `634` pixels high, and place it at `+0+0` on a transparent `2067 x 634` canvas; place the untouched `1448 x 472` canonical wordmark at `+596+88` above it so the spelling, punctuation, yellow-orange fill, red extrusion, dark outline, white highlights, arch, and spacing remain deterministic. Verify genuine alpha and exclude backgrounds, halos, extra marks, action rays, UI, and watermarks. |
...
| `apps/website/public/brand/features/cooper-hero.png` | `1399 x 1124` shared transparent Cooper cutout used by the Features-page hero and footer, About-page hero and footer, Parents-page footer, Build-page panel heading and chat avatars, and Lab-page hero and empty-project doodle, generated with the built-in image editor. Use `apps/website/public/brand/about/cooper-hero.png` as the authoritative wide-wing cheering pose, silhouette, crop, and canvas reference; use the updated `apps/website/public/brand/homepage/cooper-hero-fixed.png` plus `brand/cooper/cooper-1.png`, `brand/cooper/cooper-3.png`, and `brand/cooper/cooper-5.png` as the authoritative new character design. Preserve the open smile, front-facing head, horizontal spread wings, lower-left tail, cropped torso, blue-black goggles, white coat, blue-bordered `Cooper` badge, and red/blue/green pens. Use exactly two peach adhesive bandages crossed on top of the beak and exactly one small thin red cross-scratch on each cheek; keep the remaining face clean, allow only faint pale coat scuffs, and exclude sewn patches, other bandages or scratches, brown stains, grime, action rays, props, backgrounds, shadows, halos, borders, logos, UI, and watermarks. Preserve genuine alpha and the exact dimensions without resampling. |
...
| `apps/website/public/brand/team/cooper.png` | Runtime copy of `brand/cooper/cooper.png` for the `/team` page. Preserve the exact `1024 x 1024` PNG without resampling. |
| `apps/website/public/brand/team/rupert.png` | Runtime copy of `brand/cooper/rupert.png` for the `/team` page. Preserve the exact `1208 x 1302` PNG without resampling. |
| `apps/website/public/brand/team/jamie.png` | Runtime copy of `brand/cooper/jamie.png` for the `/team` page. Preserve the exact `1208 x 1302` PNG without resampling. |
| `apps/website/public/brand/team/vix.png` | Runtime copy of `brand/cooper/vix.png` for the `/team` page. Preserve the exact `1208 x 1302` PNG without resampling. |
| `apps/website/public/brand/team/lango.png` | Runtime copy of `brand/cooper/lango.png` for the `/team` page. Preserve the exact `1208 x 1302` PNG without resampling. |
| `apps/website/public/brand/team/leenie.png` | Runtime copy of `brand/cooper/leenie.png` for the `/team` page. Preserve the exact `1208 x 1302` PNG without resampling. |
...
| `apps/website/public/brand/parents/parents-hero-cooper-source.png` | `1337 x 1176` transparent generation master for the friendly Parents hero Cooper, generated with the built-in image editor. Use the previous Parents hero as the authoritative upright pose, silhouette, thumbs-up, clipboard, crop, and canvas reference; use the updated homepage and Features Cooper cutouts plus `brand/cooper/cooper-1.png` and `brand/cooper/cooper-5.png` as the authoritative new character design. Preserve the friendly open smile, large left-wing thumbs-up, right wing holding the tan clipboard, lower-left tail, blue-black goggles, white coat, blue-bordered `Cooper` badge, red/blue/green pens, gray clipboard clip, smile icon, and the exact four-line clipboard message `BIG / IDEAS / BRIGHTER / KIDS`. Use exactly two peach adhesive bandages crossed on top of the beak and exactly one small thin red cross-scratch on each cheek; keep the remaining face clean, allow only faint pale coat scuffs, and exclude sewn patches, other bandages or scratches, brown stains, grime, action rays, backgrounds, shadows, halos, borders, logos, UI, and watermarks. Preserve genuine alpha and the exact dimensions without resampling. |
| `apps/website/public/brand/parents/parents-hero-cooper.png` | Runtime copy of the updated friendly Parents hero Cooper generation master. Preserve the genuine alpha channel, exact `1337 x 1176` dimensions, complete thumb, clipboard, comb, lower coat, crossed beak bandages, paired cheek scratches, badge, and clipboard lettering; do not reconstruct the clipboard or facial details in HTML or CSS. |
